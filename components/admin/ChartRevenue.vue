...
const load = async () => {
  ...
  orders.value = orders.value.map((item) => {
    ...
    groups.value[new Date(item.created).toLocaleDateString("de")].push(item);
  });

  days.value = [0, 0, 0, 0, 0, 0, 0];
  for (let i = 0; i < Object.entries(groups.value).length; i++) {
    days.value[i] = calcRev(Object.values(groups.value)[i]);
  }
};
...
const calcRev = (item) => {
  ...
  for (let i = 0; i < item.length; i++) {
    for (let j = 0; j < item[i].cart.length; j++) {
      total += item[i].cart[j].qty * item[i].cart[j].product.price;
    }
  }
  return total;
};
